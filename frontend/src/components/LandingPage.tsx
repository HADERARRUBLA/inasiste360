...
export const LandingPage: React.FC<LandingPageProps> = ({ onLoginClick }) => {
    ...
    return (
      <aside className="hidden lg:flex fixed left-0 top-0 h-screen w-72 bg-surface border-r border-outline-variant/10 flex-col z-40 pt-32 pb-8 shadow-2xl">
        <div className="px-8 mb-12 flex flex-col items-center gap-6">
            <div className="text-[10px] font-bold text-primary/40 tracking-[0.2em] mb-4 w-full">NODO IA ACTIVO</div>
        </div>
    ...
    <div className="relative min-h-screen pt-20 flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="container mx-auto px-8 relative z-10 flex flex-col lg:flex-row items-center gap-20 py-24">
        <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="flex-1 space-y-10 text-center lg:text-left">
          <h1 className="text-5xl md:text-[5.5rem] lg:text-[6.5rem] font-black tracking-tight leading-[0.9] text-on-surface">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-8 pt-4 justify-center lg:justify-start">
            <button onClick={() => window.open(CTA_LINK, '_blank')} className="w-full sm:w-auto px-12 py-6 bg-[#0047ab] text-white rounded-md metallic-edge font-bold uppercase tracking-widest text-sm hover:brightness-110 active:scale-[0.98] transition-all shadow-xl">Detener las fugas de nómina hoy</button>
            <button onClick={onLoginClick} className="w-full sm:w-auto px-12 py-6 bg-surface-container-high border border-outline-variant/30 text-on-surface rounded-md font-bold uppercase tracking-widest text-sm hover:bg-surface-container-highest transition-all backdrop-blur-md">Acceso VIP</button>
          </div>
          <div className="flex items-center gap-3 pt-10">
              <div className="flex -space-x-3">
                  {[1,2,3].map(i => <div key={i} className="w-8 h-8 rounded-full border-2 border-background bg-slate-800" />)}
              </div>
              <span className="text-[10px] font-bold text-on-surface-variant/40 uppercase tracking-widest">Confiado por empresas Fortune 500 para control total.</span>
          </div>
        </motion.div>
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1 }} className="flex-1 relative w-full aspect-square max-w-[650px] flex items-center justify-center">
          <div className="relative w-full h-full rounded-2xl border border-white/5 bg-surface-container-low/40 backdrop-blur-2xl overflow-hidden shadow-2xl p-4">
            ...
            <div className="absolute top-10 right-10 p-4 bg-background/80 backdrop-blur-xl border border-white/10 rounded-lg metallic-edge w-56 shadow-xl">
               <div className="flex justify-between items-start mb-4"><span className="text-[10px] uppercase tracking-tighter text-primary font-bold">PRETCISION DEL NODO</span><span className="text-[10px] text-green-400 font-mono">99.9%</span></div>
            ...
            <div className="absolute bottom-12 left-10 p-4 bg-background/80 backdrop-blur-xl border border-white/10 rounded-lg metallic-edge shadow-xl flex items-center gap-3">
  ...
  const ROIView = () => {
    ...
    return (
      <div className="min-h-screen pt-32 pb-12 px-8 bg-background">
        <div className="max-w-7xl mx-auto">
          <header className="mb-20">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/10 border border-primary/20 rounded mb-4"><Shield className="w-4 h-4 text-primary" /><span className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">PROTOCOLO DE SEGURIDAD ACTIVO</span></div>
            <h1 className="text-6xl md:text-8xl font-black tracking-tighter text-white mb-8 leading-tight">Calculadora de <br/><span className="text-primary/80">ROI de Confianza</span></h1>
            <p className="text-xl text-on-surface-variant max-w-2xl font-light">Calcula cuánto dinero está perdiendo tu empresa por el fraude de asistencia y la ineficiencia operativa. Masteriza tus activos con inteligencia soberana.</p>
          </header>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            <div className="lg:col-span-4 space-y-12">
              <div className="glass-panel p-10 rounded-lg">
                ...
                <input type="range" min="5" max="10000" value={personnel} onChange={(e) => setPersonnel(parseInt(e.target.value))} className="w-full h-1 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-primary" />
                ...
                <input type="range" min="1" max="15" step="0.5" value={fraudRate} onChange={(e) => setFraudRate(parseFloat(e.target.value))} className="w-full h-1 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-primary" />
            ...
            <div className="lg:col-span-8 space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  ...
                  <div key={i} className="bg-surface-container-high p-8 rounded-lg border border-white/5 hover:border-primary/20 transition-all group">
                    <label className="text-[9px] font-bold uppercase tracking-[0.2em] text-slate-500 mb-6 block">{stat.label}</label>
                    <div className="flex justify-between items-center"><span className="text-3xl font-black text-white group-hover:text-primary transition-colors">{stat.value}</span><stat.icon className="w-5 h-5 text-slate-700 group-hover:text-primary/50 transition-colors" /></div>
                  </div>
                ))}
              </div>
  ...
  const DashboardView = () => (
    <div className="pl-0 lg:pl-72 pt-20 h-screen w-full bg-background overflow-auto relative">
      ...
      <div className="h-full w-full p-12 flex flex-col gap-10 relative z-10">
        <div>
            <h1 className="text-[4.5rem] font-bold tracking-[-0.04em] leading-none text-on-surface">Centro de Comando</h1>
            <p className="text-[0.6875rem] uppercase tracking-[0.2em] text-primary mt-4 font-bold">NODO OPERATIVO: GLOBAL-ALPHA-9</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="glass-panel p-10 rounded-lg relative overflow-hidden group">
            <User className="absolute top-0 right-0 p-6 opacity-10 w-16 h-16 text-primary" />
            <div className="text-[0.6875rem] font-bold tracking-[0.2em] uppercase text-on-surface-variant/60 mb-3">PERSONAL ACTIVO</div>
            <div className="flex items-baseline gap-5"><span className="text-5xl font-black tracking-tighter text-on-surface">12,540</span><div className="flex items-center gap-2 bg-primary/10 px-3 py-1 rounded-full"><span className="w-2 h-2 bg-primary rounded-full animate-pulse"></span><span className="text-[0.65rem] font-bold text-primary uppercase">Validando</span></div></div>
            <div className="mt-6 h-1.5 bg-white/5 w-full rounded-full overflow-hidden"><div className="h-full bg-primary/40 w-[85%]"></div></div>
          </div>
          <div className="glass-panel p-10 rounded-lg relative overflow-hidden group">
            <Wallet className="absolute top-0 right-0 p-6 opacity-10 w-16 h-16 text-primary" />
            <div className="text-[0.6875rem] font-bold tracking-[0.2em] uppercase text-on-surface-variant/60 mb-3">RECUPERACIÓN DE NÓMINA (YTD)</div>
            <div className="flex items-baseline gap-3"><span className="text-5xl font-black tracking-tighter text-on-surface">$1.2M</span><span className="text-[0.6875rem] font-bold text-emerald-500 font-mono tracking-tighter">~14%</span></div>
            <div className="mt-8 flex gap-2">{[1,2,3,4,5,6].map(i => <div key={i} className={`flex-1 h-3 rounded-sm ${i < 5 ? 'bg-primary/30' : 'bg-white/5'}`}></div>)}</div>
          </div>
          <div className="glass-panel p-10 rounded-lg relative overflow-hidden group">
            <Shield className="absolute top-0 right-0 p-6 opacity-10 w-16 h-16 text-primary" />
            <div className="text-[0.6875rem] font-bold tracking-[0.2em] uppercase text-on-surface-variant/60 mb-3">INTEGRIDAD DEL SISTEMA</div>
            <div className="flex items-baseline gap-4"><span className="text-5xl font-black tracking-tighter text-on-surface">99.98%</span><Activity className="w-6 h-6 text-primary animate-pulse" /></div>
            <div className="mt-6 text-[10px] text-on-surface-variant/40 font-bold uppercase tracking-widest">Tiempo de Actividad: 4,520 Horas Continuas</div>
          </div>
        </div>
        <div className="flex-1 flex gap-10 min-h-0 pb-10">
          <div className="flex-[3] glass-panel rounded-xl overflow-hidden relative bg-surface-container-lowest">
            <div className="absolute top-10 left-10 z-20"><div className="bg-background/80 backdrop-blur px-5 py-2.5 rounded-lg border border-outline-variant/20 flex items-center gap-3"><span className="w-2.5 h-2.5 rounded-full bg-primary animate-ping"></span><span className="text-[0.75rem] font-black tracking-widest uppercase text-on-surface">Malla de Nodos en Vivo</span></div></div>
            ...
            <div className="absolute bottom-10 left-10 p-6 glass-panel rounded-xl max-w-sm">
                <div className="text-[10px] font-bold text-primary mb-2 tracking-[0.1em] uppercase">Nodo de Enfoque</div>
                <div className="text-xl font-black mb-1">Centro Ciudad de México</div>
                <div className="text-[10px] text-on-surface-variant/60">1,204 Turnos Activos • 0 Anomalías</div>
            </div>
          </div>
          <div className="flex-1 space-y-8 flex flex-col min-w-[340px]">
              <div className="glass-panel rounded-xl p-8 flex flex-col h-[60%]">
                <div className="flex justify-between items-center mb-8"><h3 className="text-[0.6875rem] font-black uppercase text-on-surface-variant/80 tracking-[0.2em]">FEED DE INTELIGENCIA</h3><span className="text-[9px] text-on-surface-variant/30 font-bold uppercase px-2 py-0.5 border border-white/5 rounded">Tiempo Real</span></div>
                    ...
                    {[
                        ...
                        <div key={i} className="flex gap-4 items-start pb-4 border-b border-white/5">
                            ...
                            <div className="flex-1">
                                <div className="flex justify-between items-start mb-1 text-[11px] font-black"><span>{item.title}</span><span className="text-[9px] text-on-surface-variant/40 font-mono">{item.time}</span></div>
                                <div className="text-[9px] text-on-surface-variant/40 font-bold uppercase tracking-tighter mb-1">{item.sub}</div>
              ...
              <div className="glass-panel rounded-xl p-8 bg-red-500/5 border-red-500/20">
                  ...
                  <div className="p-4 bg-red-500/10 rounded-lg border border-red-500/30">
                      ...
                      <p className="text-[9px] font-bold text-on-surface-variant/60 leading-tight">ID Trabajador: #98219 - Intento de autenticación fuera del perímetro.</p>
                  </div>
              </div>
          </div>
        </div>
...
const Footer = () => (
    <footer className="w-full py-24 px-12 border-t border-white/5 bg-background">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-end gap-16">
        <div className="space-y-8"><span className="text-4xl font-black text-on-surface tracking-tighter uppercase">ASISTE 360</span><p className="text-[12px] font-black text-on-surface-variant/40 uppercase tracking-[0.3em] leading-relaxed max-w-sm">© 2024 ASISTE360.<br/>TECNOLOGÍA SOBERANA.</p></div>
        <div className="flex gap-16">{['Protocolo', 'Privacidad', 'Términos'].map((item) => (<a key={item} href="#" className="text-[12px] font-black uppercase tracking-[0.25em] text-on-surface-variant/30 hover:text-primary transition-all">{item}</a>))}</div>
      </div>
    </footer>
);
